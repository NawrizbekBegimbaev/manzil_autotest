...
- **BUG-038** (→ трекер: ключ будет присвоен, открыт) — гонка attach одного водителя на два заказа → двойное назначение (лок на заказ, не на водителя). test_race_attach_driver_two_orders xfail(strict).
