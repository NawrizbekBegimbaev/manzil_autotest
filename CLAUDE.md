...
**ЖЕЛЕЗНОЕ ПРАВИЛО ОТЧЁТНОСТИ.** Каждый отчёт о закрытии под-блока/модуля ОБЯЗАН содержать
**хэш коммита** и **число тестов из РЕАЛЬНОГО прогона** (`git rev-parse HEAD` + вывод pytest).
Работа без коммита в git **не существует и не отчитывается** — «готово» без хэша считается
непроверенной гипотезой, а не результатом. Никаких «принято N/N» без коммита, который это
подтверждает. Расхождение хэша/чисел с репозиторием — стоп и форензика, а не движение дальше.
