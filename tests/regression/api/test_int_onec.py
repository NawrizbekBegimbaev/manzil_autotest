...
- ДОПУЩЕНИЕ ПО СТЕНДУ: токен принят dev-стендом ЭМПИРИЧЕСКИ (валидный → доменный 404
  no-active-shipment, неверный → 401), поэтому 1С-добивка валидна на факте. Разработчик не
  уверен, для какого стенда выдан токен (dev-only или общий dev/staging — не подтверждено);
  при переносе на staging проверить приём токена отдельно.
